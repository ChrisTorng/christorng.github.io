...
另趁 2025/7/1 重設 GitHub Copilot 點數之前的這幾天，多用了 Claude Sonnet 4 + Agent mode。雖然 Agent mode 運作了非常多輪，但發現耗用點數不多，因此目前推測，可能是每送出一個 user prompt 計一次，而不是像先前猜測的，每個工具調用都計一次。因此[先前文章](/blog/2025/github-copilot-model-selection-strategy)中說的「Agent/Edit 用量很兇要小心」可能是錯的。這也代表，如果用非零點模型，可以儘量用 Agent mode 來開發，讓它一次完成多一些事。
